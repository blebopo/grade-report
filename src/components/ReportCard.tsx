...
export default function ReportCardComponent({
  ...
  const grade = gradeRanges.filter(
    (grade) => grade.min <= gradePercentage && grade.max >= gradePercentage,
  )[0];
  ...
  return (
    <div className="report-card">
      ...
      <div className="report-card__first-row">
        ...
        <span className="grade-symbol" style={{ backgroundColor: grade.color }}>
          {grade.symbol}
        </span>
        ...
        <span
          ...
          style={{ color: grade.percentageColor }}
        >
          {gradePercentage}
        </span>
      </div>
      <div className="report-card__second-row">
        <div
          ...
          style={{
            flexGrow: fillRatio,
            background: `linear-gradient(to right, ${grade.gradientFrom}, ${grade.percentageColor})`,
          }}
        ></div>
        <div
          className="total-percentage-bar"
          style={{ flexGrow: emptyRatio }}
        ></div>
      </div>
    </div>
  );
